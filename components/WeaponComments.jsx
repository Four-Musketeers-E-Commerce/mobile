...
const WeaponComments = ({ weaponId, isReviewedToggle, onEditStart }) => {
  ...
  const sortComments = useCallback((comments, userId) => {
    if (!comments) return [];
    return [...comments].sort((a, b) => {
      const aUserId = a?.users?.$id;
      const bUserId = b?.users?.$id;
  ...
  useEffect(() => {
    if (data) {
      setLocalComments(sortComments(data, currentUserId));
    }
  }, [data, currentUserId, sortComments]);
  ...
  const renderCommentBox = useCallback((item) => {
