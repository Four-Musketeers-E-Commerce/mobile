...
//     // Convert ratings into a map for quick lookup
//     const ratingsMap = new Map();
//     ratings.forEach((rating) => {
//       if (rating.users?.$id && rating.reviewId) { // Assume ratings have a unique reviewId
//         const key = `${rating.users.$id}-${rating.reviewId}`; // Composite key
//         ratingsMap.set(key, rating.rating);
//       }
//     });

//     // Merge ratings into comments
//     const mergedData = comments.map((comment) => {
//       const key = `${comment.users?.$id}-${comment.$id}`; // Composite key
//       return {
//         ...comment,
//         rating: ratingsMap.get(key) ?? null, // Use composite key for lookup
//       };
//     });
...
export async function getAllCommentsAndRatings(weaponId) {
  try {
    ...
    const mergedData = comments.map((comment) => {
      ...
      // Debug logging for this specific match
      console.log(`Matching for comment at ${comment.$createdAt}:`, {
        userId: comment.users.$id,
        commentTime: comment.$createdAt,
        availableRatings: userRatings.map(r => r.created),
        matched: matchedRating ? matchedRating.created : 'none'
      });
...
export async function editComment(weaponId, newComment) {
  try {
    const currentUser = await getCurrentUser();
...
export async function editStarRating(weaponId, newRating) {
  try {
    const currentUser = await getCurrentUser();
    if (!currentUser) throw new Error("User not authenticated");
