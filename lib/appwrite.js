...
export async function getAllCommentsAndRatings(weaponId) {
  try {
    ...
    ratings.forEach((rating) => {
      if (rating.users && rating.users.$id) {
        ratingsMap.set(rating.users.$id, rating.rating);
      }
    });
    // Merge ratings into comments
    const mergedData = comments.map((comment) => ({
      ...comment,
      rating: ratingsMap.get(comment.users?.$id) ?? null, // Use users.$id for lookup
    }));
    return mergedData;
  } catch (error) {
    console.error(error);
...
export async function editStarRating(weaponId, newRating) {
  try {
    ...
    await databases.updateDocument(
      databaseId,
      ratingsCollectionId, // Changed from commentsCollectionId
      records.documents[0].$id,
      {
        users: currentUser.$id,
        weapons: weaponId,
...
export async function deleteReview(weaponId){
  try{
    const currentUser = await getCurrentUser();
    if(!currentUser) throw Error;
    const record_1 = await databases.listDocuments(
      databaseId,
      commentsCollectionId,
      [Query.equal("users", currentUser.$id), Query.equal("weapons", weaponId)]
    )
    await databases.deleteDocument(
      databaseId,
      commentsCollectionId,
      record_1.documents[0].$id
    )
    const record_2 = await databases.listDocuments(
      databaseId,
      ratingsCollectionId,
      [Query.equal("users", currentUser.$id), Query.equal("weapons", weaponId)]
    )
